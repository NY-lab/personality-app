...
# Set the logging level for Werkzeug to WARNING
...
session_use_signer=config.getboolean('session', 'use_signer')
session_key_prefix=config['session']['key_prefix']
session_lifetime_minutes=config.getint('session', 'lifetime_minutes')
...
# Connect to the database using the loaded configurations
...
# Connect to Redis using the loaded configurations
...
class UserThread(threading.Thread):
    def __init__(self, user_id, session_id, target, args=(), kwargs={}):
        super(UserThread, self).__init__(target=target, args=args, kwargs=kwargs)
        self.user_id = user_id
        self.session_id = session_id
        self.thread_name = f"Thread-{self.session_id} ({self.user_id})"
...
def homePage():
    user_id = session.get('guest','user_id')
    session_id = session.get('session_id', 'unknown_session_id')  # Get the user's session ID
    if user_id is None:
        return redirect('/login')  #
    
    
    # Example of logging from an asynchronous thread with user-specific thread name
    def log_message(message):
        thread = UserThread(user_id, session_id, target=logger.info, args=(message,))
        thread.start()

    # Log the user's access to the career page
    log_message('Accessed the HomePage.')

    logger.debug('Session Data after login: %s', session)
    logger.debug('Guest user login successful.')
...
def career():
    ...
    # Log the user's access to the career page
    log_message('Accessed the career page.')
...
# Function to log messages from user-specific threads
def log_message(message):
    user_id = session.get('user_id', 'guest')
    session_id = session.get('session_id', 'unknown_session_id')  # Get the user's session ID
    thread = UserThread(user_id, session_id, target=logger.info, args=(message,))
...
def login():
    ...
    if request.method == 'POST':
        ...
        if username == GUEST_USERNAME and password == GUEST_PASSWORD:
            ...
            log_message('Guest user login successful.')
        ...
        try:
            if user_data and hashlib.sha256(password.encode()).hexdigest() == user_data['password']:
                ...
                log_message(f'Login failed for user: {username}')
...
def register():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            values = (username, hashed_password, email, timestamp)  # Use hashed password here
